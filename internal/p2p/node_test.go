...
err = s1.Publish(newMessage("unit"))
assert.NoError(t, err)

// Message should be received on both nodes:
waitForMessage(t, s1.Next(), newMessage("unit"))
waitForMessage(t, s2.Next(), newMessage("unit"))
waitForMessage(t, s3.Next(), newMessage("unit"))
}
